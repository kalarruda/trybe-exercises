...
SELECT cidade.city, cidade.country_id, pais.country
FROM sakila.city AS cidade
INNER JOIN sakila.country AS pais
ON cidade.country_id = pais.country_id ORDER BY country_id DESC;
...
SELECT S.first_name, S.last_name, AVG(P.amount) AS MEDIA, COUNT(P.payment_date) AS QTD_2006
FROM sakila.staff AS S
...
FROM sakila.actor AS A
	INNER JOIN 
sakila.film AS FA ON FA.actor_id = A.actor_id
	INNER JOIN 
sakila.film_actor AS F ON FA.actor_id = F.actor_id;
